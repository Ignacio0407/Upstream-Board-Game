...
@Repository
public interface MessageRepository extends BaseRepository<Message, MessageDTO, Integer> {

    @Override
    @Query("SELECT new es.us.dp1.l4_01_24_25.upstream.chat.MessageDTO(m.id, m.player.id, m.player.name, m.match.id, m.content) FROM Message m WHERE m.id = :id")
    Optional<MessageDTO> findByIdAsDTO(Integer id);

    @Override
    @Query("SELECT new es.us.dp1.l4_01_24_25.upstream.chat.MessageDTO(m.id, m.player.id, m.player.name, m.match.id, m.content) FROM Message m")
