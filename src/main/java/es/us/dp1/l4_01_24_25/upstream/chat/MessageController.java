...
public class MessageController {
    ...
    public ResponseEntity<Message> createMessage(@PathVariable Integer matchId, @PathVariable Integer playerId, @PathVariable String message) {
            ...
            if (playerService.findById(playerId) == null) throw new ResourceNotFoundException("Error encontrando al jugador");
            if (matchService.findById(matchId) == null) throw new ResourceNotFoundException("Error encontrando al jugador");

            Message createdMessage = messageService.create(playerId, matchId, message);
    ...
    public ResponseEntity<List<Message>> getMatchMessages(@PathVariable Integer matchId) {
        if (matchService.findById(matchId) == null) throw new ResourceNotFoundException("Ha habido un error encontrando la partida");
    ...
    public ResponseEntity<List<Message>> getUserMessages(@PathVariable Integer userId) {
        if (userService.findById(userId) == null) throw new ResourceNotFoundException("Ha habido un error encontrando al usuario");
